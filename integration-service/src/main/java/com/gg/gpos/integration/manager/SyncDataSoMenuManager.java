			...
			soCategoryFoodGroups.stream().forEach(scfg ->{
				...
				if(!foodGroupCodeChanged.contains(foodGroupCode)) {
					...
					if(delFoodGroup != null) {
						List<FoodGroup> foodGroupChilds = foodGroupRepository.findByParent(delFoodGroup);
						if (!CollectionUtils.isEmpty(foodGroupChilds)) {
							foodGroupChilds.stream().forEach(item ->{
								item.setParent(null);
								foodGroupRepository.save(item);
							});
						}
