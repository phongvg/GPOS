...
@Component
public class SyncMapperImpl implements SyncMapper {

    @Override
    public Sync dtoToEntity(SyncDto syncDto) {
        if ( syncDto == null ) {
            return null;
        }
    ...
    @Override
    public SyncDto entityToDto(Sync sync) {
        if ( sync == null ) {
            return null;
        }
        ...
        syncDto.setTmpSyncs( tmpSyncListToTmpSyncDtoList( sync.getTmpSyncs() ) );
    ...
    @Override
    public TmpSyncDto tmpSyncToTmpSyncDto(TmpSync tmpSync) {
        if ( tmpSync == null ) {
            return null;
        }
        ...
        tmpSyncDto.setSync( null );

        return tmpSyncDto;
    }

    protected TmpSync tmpSyncDtoToTmpSync(TmpSyncDto tmpSyncDto) {
        if ( tmpSyncDto == null ) {
            return null;
        }
        ...
        tmpSync.setSync( dtoToEntity( tmpSyncDto.getSync() ) );
    ...
    protected List<TmpSync> tmpSyncDtoListToTmpSyncList(List<TmpSyncDto> list) {
        ...
        for ( TmpSyncDto tmpSyncDto : list ) {
            list1.add( tmpSyncDtoToTmpSync( tmpSyncDto ) );
        }

        return list1;
    }

    protected List<TmpSyncDto> tmpSyncListToTmpSyncDtoList(List<TmpSync> list) {
        if ( list == null ) {
            return null;
        }

        List<TmpSyncDto> list1 = new ArrayList<TmpSyncDto>( list.size() );
        for ( TmpSync tmpSync : list ) {
            list1.add( tmpSyncToTmpSyncDto( tmpSync ) );
        }
