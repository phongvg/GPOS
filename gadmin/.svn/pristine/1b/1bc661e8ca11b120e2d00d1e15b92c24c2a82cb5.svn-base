var coFoodItemForm = function() {
    ...
    var _componentInputPreviewForm = function() {
        ...
        if (quarterPhotoUrl) {
        	...
        	var quarter = quarterPhotoUrl.slice(quarterPhotoUrl.lastIndexOf("\\") + 1);
        	quarterPhotoName = quarter.slice(quarter.lastIndexOf("/") + 1);
        } else {
        ...
        if (groupPhotoUrl) {
        	...
        	var group = groupPhotoUrl.slice(groupPhotoUrl.lastIndexOf("\\") + 1);
        	groupPhotoName = group.slice(group.lastIndexOf("/") + 1);
        } else {
    ...
    var delKaloGroup = function(){
		$('#delKaloGroup').on('click', function(){
			$('#kaloGroupId').val('').trigger('change');
		})
	}
