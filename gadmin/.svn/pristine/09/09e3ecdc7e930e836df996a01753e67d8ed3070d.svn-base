...
@Slf4j
@Controller
public class OrderVoidController extends BaseController {
	@Autowired
	...
	@GetMapping("/ref/orderVoid/list")
    public ModelAndView list() {
		log.info("ENTERING 'LIST ORDER-VOID' METHOD...");
		ModelAndView modelAndView = new ModelAndView("ref/order-void-list");
		OrderVoidDto criteria = new OrderVoidDto();
		criteria.setPage(appProperties.getDefaultPage());
		criteria.setSize(appProperties.getDefaultPageSize());
		Page<OrderVoidDto> pages = orderVoidManager.gets(criteria);
		modelAndView.addObject("page", pages);
		modelAndView.addObject("criteria",criteria);
        return modelAndView;
    }
    
    @PostMapping("/ref/orderVoid/list")
    public ModelAndView search(Model model, @Valid OrderVoidDto criteria, BindingResult bindingResult){
    	log.info("ENTERING 'SEARCH ORDER-VOID' METHOD...");
		ModelAndView modelAndView = new ModelAndView("ref/order-void-list");
     ...
     */
    @GetMapping("/orderVoid/sync-from-rk7")
    public String syncFromRK7(Model model, HttpServletRequest request) {
    	log.info("ENTERING 'SYNC ORDER_VOID FROM RK7' METHOD...");
    	try {
        	...
        	addMessage(request, getText("orderVoid.sync.from.rk7.success", request.getLocale()));
		} catch (Exception e) {
			addError(request, e.getMessage());
			log.error("SYNC ORDER_VOID FROM RK7 EXCEPTION: {}", e);
		}
    	return "redirect:/ref/orderVoid/list";
    }
