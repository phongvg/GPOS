...
public class EmployeeController extends BaseController {
    ...
    @GetMapping("/ref/employee/list")
    public ModelAndView list() {
    	log.info("ENTERING 'LIST EMPLOYEE' METHOD...");
    	ModelAndView modelAndView = new ModelAndView("ref/employee-list");
    	EmployeeDto criteria = new EmployeeDto();
    	criteria.setPage(appProperties.getDefaultPage());
    	criteria.setSize(appProperties.getDefaultPageSize());
    	Page<EmployeeDto> pages = employeeManager.gets(criteria);
    	modelAndView.addObject("page", pages);
    	modelAndView.addObject("criteria", criteria);
    	return modelAndView;
    }
    
    @PostMapping("/ref/employee/list")
    public ModelAndView search(Model model, @Valid EmployeeDto criteria, BindingResult bindingResult) {
    	log.info("ENTERING 'SEARCH EMPLOYEE' METHOD...");
    	ModelAndView modelAndView = new ModelAndView("ref/employee-list");
     ...
     */
    @GetMapping("/employee/sync-from-rk7")
    public String syncFromRK7(Model model, HttpServletRequest request) {
    	log.info("ENTERING 'SYNC EMPLOYEE FROM RK7' METHOD...");
    	try {
        	...
        	addMessage(request, getText("employee.sync.from.rk7.success", request.getLocale()));
		} catch (Exception e) {
			addError(request, e.getMessage());
			log.error("SYNC EMPLOYEE FROM RK7 EXCEPTION: {}", e);
		}
    	return "redirect:/ref/employee/list";
    }   
