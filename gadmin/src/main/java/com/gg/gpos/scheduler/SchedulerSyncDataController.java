...
public class SchedulerSyncDataController {
    ...
    private void schedulingSyncData() {
		...
		List<SyncDto> syncDtoHasStatusWaitForSyncNow = syncManager.getTop5ByStatusAndTypeSyncNot(SyncStatusEnum.WAIT_FOR_SYNC_NOW.val, TypeSyncEnum.SYNC_FILE_DATA_TO_SERVER_RESTAURANT.val);
    	if(!CollectionUtils.isEmpty(syncDtoHasStatusInProcessings) || !CollectionUtils.isEmpty(syncDtoHasStatusWattings) || !CollectionUtils.isEmpty(syncDtoHasStatusWaitForSyncNow)) {
    		threadSyncManager.syncData(syncDtoHasStatusWattings, syncDtoHasStatusInProcessings, syncDtoHasStatusWaitForSyncNow);
    	}
    	// Đồng bộ hình ảnh
		...
		List<SyncDto> syncDtoTypeFileDataHasStatusWaitForSyncNow = syncManager.getTop5ByStatusAndTypeSync(SyncStatusEnum.WAIT_FOR_SYNC_NOW.val, TypeSyncEnum.SYNC_FILE_DATA_TO_SERVER_RESTAURANT.val);
    	if(!CollectionUtils.isEmpty(syncDtoTypeFileDataHasStatusWattings) || !CollectionUtils.isEmpty(syncDtoTypeFileDataHasStatusInProcessings) || !CollectionUtils.isEmpty(syncDtoTypeFileDataHasStatusWaitForSyncNow)) {
    		threadSyncFileManager.syncFile(syncDtoTypeFileDataHasStatusWattings, syncDtoTypeFileDataHasStatusInProcessings, syncDtoTypeFileDataHasStatusWaitForSyncNow);
    	}
    }
}
