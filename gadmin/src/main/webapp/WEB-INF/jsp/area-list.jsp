...
<!-- Content area -->
<div class="content">
	<form:form id="areaForm" modelAttribute="criteria" action="${ctx}/area/list?kId=${criteria.kds.id}&rCode=${criteria.restaurantCode}"  method="post">
			...
			<div class="card">
				...
				<security:authorize access="hasRole('ROLE_RES_CONFIG_EDIT')">
					<div class="list-icons" style="float: right">
						<a href="<c:url value='/area/form?rCode=${criteria.restaurantCode}'/>" class="btn btn-sm btn-primary" title="<fmt:message key="button.add"/>" style=" margin-top: -25%; margin-bottom: 15%"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
	               	</div>
	            </security:authorize>
		               	...
		               	<table class="table table-bordered table-striped">
		               		<thead>
		                		<tr class="table-success">
		                			<th>#</th>
		                			<th><fmt:message key="area.printPosCode"/></th>
		                			<th><fmt:message key="area.name"/></th>
		                			<th><fmt:message key="printer"/></th>
		                			<th><fmt:message key="area.type"/></th>
		               		...
		               		<tbody>
			               		...
			               		<c:forEach items="${page.content }" var="area" varStatus="cnt">
			                		<tr>
		                				<td><c:out value="${cnt.count+page.size*page.number}"></c:out></td>
		                				<td>${area.printPosCode }</td>
			                			<td>${area.hallplan.name }</td>
			                			<td>${area.printerName }</td>
			                			<td>${area.type }</td>
			                			<td class="text-left">
											<div class="list-icons">
						                		<a href="<c:url value='/area/form?id=${area.id}&rCode=${area.restaurantCode}'/>" class="list-icons-item text-primary-600" title="<fmt:message key='button.edit'/>" data-container="body"><i class="icon-pencil7"></i></a>
													...
													<div class="modal-dialog modal-md">
														<div class="modal-content">
															...
															<div class="modal-body">
																<p><fmt:message key="organization.confirm.success"/> <span style="color: blue;">${area.name}</span></p>
															</div>
															<div class="modal-footer">
																<button type="button" class="btn btn-default" data-dismiss="modal"><fmt:message key="button.cancel"/></button>
																<a href="<c:url value='/area/delete?id=${area.id}' />" id="confirm"  class="btn btn-primary"><fmt:message key="button.confirm"/></a>
															</div>
														</div>
													</div>
												</div>
