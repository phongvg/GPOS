...
<head>
	...
	<script src="<c:url value='/themes/admin/assets/js/kds_config_cooking_list.js'/>"></script>
</head>


<!-- Content area -->
<div class="content">
	<form:form id="kdsConfigCookingForm" modelAttribute="criteria" action="${ctx}/res/kdsConfigCooking/list"  method="post">
		...
		<div class="tab-content">
			...
			<div class="card">
				...
				<ul class="nav nav-tabs">
					<li class="nav-item"><a href="<c:url value="/res/kdsPlace/list?rCode=${criteria.restaurantCode}"/>" class="nav-link"><fmt:message key="tab.kds.place"/></a></li>
					...
					<li class="nav-item"><a href="<c:url value="/res/kdsConfigCooking/list?rCode=${criteria.restaurantCode}"/>" class="nav-link active"><fmt:message key="tab.kds.config.cooking"/></a></li>
				</ul>
				<div class="card-body">
					<div class="card-header header-elements-inline">
						<span class="text-uppercase font-size-lg font-weight-bold"><fmt:message key="tab.kds.config.cooking"/></span>
						<div class="header-elements">
							<div class="list-icons">
								<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CO_ALL,ROLE_CO_IMPORT_EXCEL')">
					  			...
					  			<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CO_ALL,ROLE_CO_EXPORT_EXCEL')">
						  			<a type="button" class="btn btn-light" href="<c:url value='/export/kdsConfigCooking?rCode=${criteria.restaurantCode}'/>" id="btnExport"> <i class="icon-cloud-upload2"></i> <fmt:message key="button.export"/></a>
					  			</security:authorize>
								<a href="<c:url value='/res/kdsConfigCooking/form?rCode=${criteria.restaurantCode}'/>" class="btn btn-sm btn-primary" title="<fmt:message key="button.add"/>"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
			               	</div>
						</div>
					</div>
		               	...
		               	<table class="table table-bordered table-striped">
		               		<thead>
		                		<tr class="table-success">
		                			...
		                			<th><fmt:message key="menu.foodItem"/></th>
		                			<th><fmt:message key="kds.config.cooking.single.cooking.time"/></th>
		                			<th><fmt:message key="kds.config.cooking.multi.cooking.time"/></th>
		               		...
		               		<tbody>
			               		...
			               		<c:forEach items="${page.content }" var="item" varStatus="cnt">
			                		<tr>
			                			...
			                			<td><c:out value="${item.foodItem.code} - ${item.foodItem.name}"></c:out></td>
			                			<td><c:out value="${item.singleCookingTime}"></c:out></td>
			                			<td><c:out value="${item.multiCookingTime}"></c:out></td>
			                			<td><c:out value="${item.kdsPlace.code} - ${item.kdsPlace.name}"></c:out></td>
			                			<td class="text-center">
											<div class="list-icons">
						                		<a href="<c:url value='/res/kdsConfigCooking/form?id=${item.id}&rCode=${item.restaurantCode}'/>" class="list-icons-item text-primary-600" title="<fmt:message key='button.edit'/>" data-container="body"><i class="icon-pencil7"></i></a>
...
<!-- modal import -->
<form:form id="kdsPlaceImport" action="${ctx}/import/kdsConfigCooking" method="post" modelAttribute="kdsConfigCooking" enctype="multipart/form-data">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<a href="/themes/admin/template_export/Kds_Config_Cooking.xlsx" id="downloadFileCheck" download><fmt:message key="button.download.file.template.export"/></a>
