...
<%-- 	<script src="<c:url value='/themes/admin/global_assets/js/plugins/extensions/jquery_ui/interactions.min.js'/>"></script> --%>
	...
	<script src="<c:url value='/themes/admin/assets/js/print_group_list.js'/>"></script>
</head>

<!-- Content area -->
<div class="content">
	<form:form id="areaForm" modelAttribute="criteria" action="${ctx}/printGroup/list?kId=${criteria.kds.id}&rCode=${criteria.restaurantCode}"  method="post">
			...
			<p class="mb-4"><fmt:message key="kitchen.fax.title.description"/></p>
				...
				<fieldset class="mb-3">
					...
					<ul class="nav nav-tabs nav-tabs-bottom nav-justified">
						...
						<li class="nav-item"><a href="<c:url value="/kds/form?kId=${criteria.kds.id}&rCode=${criteria.restaurantCode}"/>" class="nav-link active"><i class="icon-tree7 mr-2"></i><fmt:message key="tab.kitchen.fax"/></a></li>
		...
		<div class="tab-content">
			...
			<div class="card">
				...
				<ul class="nav nav-tabs">
					<li class="nav-item"><a href="<c:url value='/kds/form?rCode=${criteria.restaurantCode}'/>" class="nav-link"><fmt:message key="tab.kitchen.fax"/></a></li>
					<li class="nav-item"><a href="<c:url value="/area/list?rCode=${criteria.restaurantCode}"/>" class="nav-link"><fmt:message key="tab.kds.area"/></a></li>
					<li class="nav-item"><a href="<c:url value="/kitchen/list?rCode=${criteria.restaurantCode}"/>" class="nav-link"><fmt:message key="tab.kds.kitchen"/></a></li>
					<li class="nav-item"><a href="#" class="nav-link active"><fmt:message key="tab.kds.printGroup"/></a></li>
					<li class="nav-item"><a href="<c:url value="/res/kitchenType/list?rCode=${criteria.restaurantCode}"/>" class="nav-link"><fmt:message key="tab.kds.kitchenType"/></a></li>
				</ul>
				<div class="card-body">
					<div class="card-header header-elements-inline">
						<span class="text-uppercase font-size-lg font-weight-bold"><fmt:message key="tab.kds.printGroup"/></span>
						<div class="header-elements">
							<div class="list-icons">
								<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CO_ALL,ROLE_CO_IMPORT_EXCEL')">
					  			...
					  			<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CO_ALL,ROLE_CO_EXPORT_EXCEL')">
						  			<a type="button" class="btn btn-light" href="<c:url value='/export/printGroup?resCode=${criteria.restaurantCode}'/>" id="btnExport"> <i class="icon-cloud-upload2"></i> <fmt:message key="button.export"/></a>
					  			</security:authorize>
								<a href="<c:url value='/printGroup/form?rCode=${criteria.restaurantCode}'/>" class="btn btn-sm btn-primary" title="<fmt:message key="button.add"/>"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
			               	</div>
						</div>
					</div>
					...
					<div class="list-icons" style="float: right">
						<a href="<c:url value='/printGroup/form?rCode=${criteria.restaurantCode}'/>" class="btn btn-sm btn-primary" title="<fmt:message key="button.add"/>" style=" margin-top: -25%; margin-bottom: 15%"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
	               	</div>
	               	</security:authorize> --%>
		                		...
		                		<c:forEach items="${page.content }" var="printGroup" varStatus="cnt">
		                			<tr>
			                			...
			                			<td class="text-center">
											<div class="list-icons">
						                		<a href="<c:url value='/printGroup/form?id=${printGroup.id}&rCode=${printGroup.restaurantCode}'/>" class="list-icons-item text-primary-600" title="<fmt:message key='button.edit'/>" data-container="body"><i class="icon-pencil7"></i></a>
...
<!-- /content area -->
<!-- modal import -->
<form:form id="printGroupImport" action="${ctx}/import/printGroup" method="post" modelAttribute="printGroupDto" enctype="multipart/form-data">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<a href="/themes/admin/template_export/Print_Group.xlsx" id="downloadFileCheck" download><fmt:message key="button.download.file.template.export"/></a>
