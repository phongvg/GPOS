...
<!-- Content area -->
<div class="content">
	<form:form id="areaForm" modelAttribute="criteria" action="${ctx}/kitchen/list?rCode=${criteria.restaurantCode}"  method="post">
		...
		<div class="tab-content">
			...
			<div class="card">
				...
				<ul class="nav nav-tabs">
					...
					<li class="nav-item"><a href="<c:url value="/printGroup/list?rCode=${criteria.restaurantCode}"/>" class="nav-link"><fmt:message key="tab.kds.printGroup"/></a></li>
				...
				<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_RES_ALL,ROLE_RES_CONFIG_EDIT')">
					<div class="list-icons" style="float: right">
						<a href="<c:url value='/kitchen/form?rCode=${criteria.restaurantCode}'/>" class="btn btn-sm btn-primary" title="<fmt:message key="button.add"/>" style=" margin-top: -25%; margin-bottom: 15%"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
	               	</div>
	            </security:authorize>
		               	...
		               	<table class="table table-bordered table-striped">
		               		<thead>
		                		<tr class="table-success">
		                			<th>#</th>
		                			<th><fmt:message key="kitchen.code"/></th>
		               		...
		               		<tbody>
		               			...
		               			<c:forEach items="${page.content }" var="kitchen" varStatus="cnt">
		                			<tr>
		                				<td><c:out value="${cnt.count+page.size*page.number}"></c:out></td>
			                			<td><c:out value="${kitchen.kitchenCode }"></c:out></td>
			                			<td><c:out value="${kitchen.kitchenType }"></c:out></td>
			                			<td><c:out value="${kitchen.printName }"></c:out></td>
			                			<td>
			                				<c:forEach var="hallplan" items="${kitchen.hallplans}">
			                					<c:out value="${hallplan.name},"></c:out>
			                				</c:forEach>
			                			</td>
			                			<td class="text-center">
											<div class="list-icons">
						                		<a href="<c:url value='/kitchen/form?id=${kitchen.id}&rCode=${kitchen.restaurantCode}'/>" class="list-icons-item text-primary-600" title="<fmt:message key='button.edit'/>" data-container="body"><i class="icon-pencil7"></i></a>
													...
													<div class="modal-dialog modal-md">
														<div class="modal-content">
															...
															<div class="modal-body">
																<p class="text-left"><fmt:message key="organization.confirm.success"/> <span style="color: blue;"><c:out value="${kitchen.kitchenCode}"></c:out></span></p>
															</div>
															<div class="modal-footer">
																<button type="button" class="btn btn-default" data-dismiss="modal"><fmt:message key="button.cancel"/></button>
																<a href="<c:url value='/kitchen/delete?id=${kitchen.id}'/>" id="confirm"  class="btn btn-primary"><fmt:message key="button.confirm"/></a>
															</div>
														</div>
													</div>
												</div>
