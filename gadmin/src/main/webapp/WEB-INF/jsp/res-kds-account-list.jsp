...
<head>
	...
	<script src="<c:url value='/themes/admin/assets/js/kds_account_list.js'/>"></script>
</head>


<!-- Content area -->
<div class="content">
	<form:form id="kdsAccountForm" modelAttribute="criteria" action="${ctx}/res/kdsAccount/list"  method="post">
		...
		<div class="tab-content">
			...
			<div class="card">
				...
				<ul class="nav nav-tabs">
					...
					<li class="nav-item"><a href="<c:url value="/res/kdsAccount/list?rCode=${criteria.restaurantCode}"/>" class="nav-link active"><fmt:message key="tab.kds.account"/></a></li>
				...
				<div class="card-body">
					<div class="card-header header-elements-inline">
						<span class="text-uppercase font-size-lg font-weight-bold"><fmt:message key="tab.kds.account"/></span>
						<div class="header-elements">
							<div class="list-icons">
								<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CO_ALL,ROLE_CO_IMPORT_EXCEL')">
					  			...
					  			<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CO_ALL,ROLE_CO_EXPORT_EXCEL')">
						  			<a type="button" class="btn btn-light" href="<c:url value='/export/kdsAccount?rCode=${criteria.restaurantCode}'/>" id="btnExport"> <i class="icon-cloud-upload2"></i> <fmt:message key="button.export"/></a>
					  			</security:authorize>
								<a href="<c:url value='/res/kdsAccount/form?rCode=${criteria.restaurantCode}'/>" class="btn btn-sm btn-primary" title="<fmt:message key="button.add"/>"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
			               	</div>
						</div>
					</div>
		               	...
		               	<table class="table table-bordered table-striped">
		               		<thead>
		                		<tr class="table-success">
		                			...
		                			<th><fmt:message key="kds.account.name"/></th>
		                			<th><fmt:message key="kds.account.password"/></th>
		               		...
		               		<tbody>
			               		...
			               		<c:forEach items="${page.content }" var="item" varStatus="cnt">
			                		<tr>
			                			...
			                			<td>${item.password }</td>
			                			<td>
			                				<c:forEach var="kdsPlace" items="${item.kdsPlaces}">
			                					<c:out value="${kdsPlace.name},"></c:out>
			                				</c:forEach>
			                			</td>
			                			<td class="text-center">
											<div class="list-icons">
						                		<a href="<c:url value='/res/kdsAccount/form?id=${item.id}&rCode=${item.restaurantCode}'/>" class="list-icons-item text-primary-600" title="<fmt:message key='button.edit'/>" data-container="body"><i class="icon-pencil7"></i></a>
...
<!-- modal import -->
<form:form id="kdsPlaceImport" action="${ctx}/import/kdsAccount" method="post" modelAttribute="kdsAccount" enctype="multipart/form-data">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<a href="/themes/admin/template_export/Kds_Account.xlsx" id="downloadFileCheck" download><fmt:message key="button.download.file.template.export"/></a>
