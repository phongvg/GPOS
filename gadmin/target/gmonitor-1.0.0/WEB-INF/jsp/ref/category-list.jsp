...
<head>
	...
	<script src="<c:url value='/themes/admin/assets/js/ref/category_list.js'/>"></script>
</head>


<div class="content">
	<form:form id="categoryForm" modelAttribute="criteria" action="${ctx}/ref/category/list"  method="post">
    <!-- \Searching -->
	<div class="card">
		...
		<div class="card-body">
			<div class="row">
				<div class="col-md-4 col-sm-12">
				   ...
				   <input class="form-control" type="text" value="${criteria.code}" name="code" placeholder="Nhập mã category..."/>
				</div>
				<div class="col-md-4 col-sm-12">
				   <label><fmt:message key="category.name" /></label>
				   <input class="form-control" type="text" value="${criteria.name}" name="name" placeholder="Nhập tên category..."/>
				</div>
				<div class="col-md-4 col-sm-12">
	...
	<div class="card">
		<div class="card-header header-elements-inline">
			<span class="text-uppercase font-size-lg font-weight-bold"><fmt:message key="category.list.title"/></span>
			<div class="header-elements">
				<div class="list-icons">
					<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CATEGORY_SYNC,ROLE_CATEGORY_ALL')">
						<a href="<c:url value='/category/sync-from-rk7'/>" id="btnSyncFromRk7" class="btn btn-sm btn-primary" ><i class="icon-download mr-2"></i><fmt:message key="button.sync.from.server"/></a>
					</security:authorize>
					<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CATEGORY_RES_SYNC,ROLE_CATEGORY_ALL')">
		...
		<div class="card-body"><fmt:message key="category.list.title.description"/></div>
		<div class="table-responsive">
			<table class="table table-bordered table-striped">
				...
				<tbody>
					...
					<c:forEach items="${page.content}" var="category" varStatus="cnt">
						<tr>
							...
							<td class="text-center"><c:if test="${category.status ne null}"><span class="badge ${(category.status eq 3) ? 'badge-primary' : 'badge-warning'}"><fmt:message key="category.status.${category.status}"/></span></c:if></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
