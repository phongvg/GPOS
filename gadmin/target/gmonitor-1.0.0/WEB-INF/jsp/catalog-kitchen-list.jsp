...
<form:form id="areaForm" modelAttribute="criteria" action="${ctx}/area/catalog-list?kId=${criteria.kds.id}"  method="post">
			...
			<p class="mb-4"><fmt:message key="kds.form.title.description"/></p>
				...
				<fieldset class="mb-3">
					...
					<ul class="nav nav-tabs nav-tabs-bottom nav-justified">
						...
						<li class="nav-item"><a href="<c:url value="/printGroup/catalog-list?kId=${criteria.kds.id}"/>" class="nav-link"><fmt:message key="tab.kds.printGroup"/></a></li>
			...
			<div class="card">
				...
				<div class="card-body">
					<div class="list-icons" style="float: right">
						<a href="<c:url value='/kitchen/catalog-form?kId=${criteria.kds.id}'/>" class="btn btn-sm btn-primary" title="<fmt:message key="button.add"/>" style=" margin-top: -25%; margin-bottom: 15%"><i class="icon-plus22"></i><fmt:message key="button.add"/></a>
	               	</div>
		               		...
		               		<tbody>
		               			...
		               			<c:forEach items="${page.content }" var="kitchen" varStatus="cnt">
		                			<tr>
			                			...
			                			<td>${kitchen.kitchenCode }</td>
			                			<td>${kitchen.kitchenType }</td>
			                			<td>${kitchen.printName }</td>
			                			<td>
			                				<c:forEach var="area" items="${kitchen.areas}">
			                					<c:out value="${area.name}">, </c:out>
			                				</c:forEach>
			                			</td>
			                			<td class="text-center">
											<div class="list-icons">
						                		<a href="<c:url value='/kitchen/catalog-form?id=${kitchen.id}&kId=${kitchen.kds.id }'/>" class="list-icons-item text-primary-600" title="<fmt:message key='button.edit'/>" data-container="body"><i class="icon-pencil7"></i></a>
													...
													<div class="modal-dialog modal-md">
														<div class="modal-content">
															...
															<div class="modal-body">
																<p class="text-left"><fmt:message key="organization.confirm.success"/> <span style="color: blue;">${kitchen.kitchenCode}</span></p>
															</div>
