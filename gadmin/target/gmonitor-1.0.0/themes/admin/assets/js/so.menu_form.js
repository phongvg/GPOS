    ...
    var initModal = function() {
		...
		const selectedFoodItems = FoodItemSelector.getSelectedFoodItems();
 		...
 		if(fileName != null && fileName != '' && imageNameOld != fileName){
 			$.ajax({
                ...
                success: function (data) {
                	if(data == 'true'){
                		...
                		createFoodGroupButton(foodGroupId, foodGroupCode, foodGroupName, foodGroupOrder, menuTypeId, menuTypeName, foodGroupLevelId, foodGroupLevelName, foodGroupNameEn, foodGroupParentId, selectedFoodItems, arrText, arrValue, avatarUrl, avatarAbsolutePath, fileName, srcImage,soId,foodGroupCodeOld,foodGroupParentName);
                	}else{
                		bootbox.alert({
        				    title: "Thông báo:",
			 ...
			 createFoodGroupButton(foodGroupId, foodGroupCode, foodGroupName, foodGroupOrder, menuTypeId, menuTypeName, foodGroupLevelId, foodGroupLevelName, foodGroupNameEn, foodGroupParentId, selectedFoodItems, arrText, arrValue, avatarUrl, avatarAbsolutePath, imageNameOld, srcImage,soId,foodGroupCodeOld,foodGroupParentName);
		}
 	});
	...
	var submitForm = function() {
		$('#btnSubmit').on('click', function(){
			...
			if(soCategoryId != null && soCategoryStatus !== true){
				bootbox.confirm({
					title: "Xác nhận:",
					message: "Việc off menu ở đanh mục sẽ đồng thời off menu mà các nhà hàng đang áp dụng danh mục này và gửi dữ liệu xuống server nhà hàng. Bạn có chắc chắn muốn off danh mục này không?",
					buttons: {
						cancel: {
...
var FoodGroup = {
	createRow: (index, foodGroupId, foodGroupCode, foodGroupName, foodGroupOrder, menuTypeId, menuTypeName, foodGroupParentId, foodGroupParentName,foodGroupLevelId,foodGroupLevelName,foodGroupNameEn, selectedFoodItems,avatarAbsolutePath,srcImage,icon,enableDeleted,changed) => {
        ...
        cols += '<td><span class="no">'+counter+'</span>&nbsp;'+spanIcon+'<input id="foodGroupId'+index+'" type="hidden" name="foodGroups['+index+'].id" value="'+foodGroupId+'"></td>';
        ...
        cols += '<td>'+foodGroupOrder+'<input id="foodGroupOrder'+index+'" type="hidden" name="foodGroups['+index+'].groupOrder" value="'+foodGroupOrder+'"><input type="hidden" id="foodGroupChanged'+index+'" name="foodGroups['+index+'].changed" value="'+changed+'"></td>';
	...
	deleteRow: (rowIndex, foodGroupId, foodGroupCode, foodGroupName, foodGroupOrder, menuTypeId, menuTypeName, foodGroupParentId, foodGroupParentName,foodGroupLevelId,foodGroupLevelName,foodGroupNameEn,avatarAbsolutePath,srcImage, icon) => {
		var index = rowIndex - 1;
	...
	loadFoodGroups: function() {
		...
		if (soCategoryId) {
			$.ajax({
				...
				method:'GET',
				data:{soId:soId, scId:soCategoryId},
				success: function(data) {
	...
	loadFoodGroupParents: (scId,soId, foodGroupCode) => {
		var contextPath = getContext();
    	$.ajax({
    		type: "GET",
    		url: contextPath + "/api/so/get-food-group-parents",
    		data: {soId:soId, fgCode: foodGroupCode,scId: scId},
    		success: function(data) {
	...
	showForm: (foodGroupCode, rowIndex) => {
		...
		if (foodGroup) {
			...
			FoodGroup.loadFoodGroupParents(scId,soId, foodGroup.foodGroupCode);
        	...
        	$.ajax({
        		...
        		data: {scId:scId, fgCode:foodGroupCode},
        		success: function(data){
        			if (data && data.foodGroupId) {
                    	...
                    	FoodGroup.loadFoodGroupParents(scId,soId, foodGroupCode);
			...
			FoodGroup.loadFoodGroupParents(scId,soId);
			previewFile(null);
		}
